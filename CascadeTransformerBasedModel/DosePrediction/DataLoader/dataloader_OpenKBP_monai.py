...
def prepare_data(files, state, cv, a_min, a_max, cache_num, cache, crop_flag, image_size):
    ...
    if cache:
        ...
        ds = CacheDataset(data=files, transform=transforms,
                          cache_num=cache_num, cache_rate=config.CACHE_RATE, 
...
def get_dataset(path, size, state="train", cv=False, a_min=-1024, a_max=1500, cache_num=24, cache=False,
    ...
    print(f"📂 Expected dataset path: {config.MAIN_PATH + config.TRAIN_DIR}")
...
def test():
    ...
    train_path = 'provided-data/train-pats/pt_*'  # 200
    test_path = 'provided-data/test-pats/pt_*'  # 100
